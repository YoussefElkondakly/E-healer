...
import '../../../BussinessLogicLayer/Doctor/Registeration/update_profile_doc_cubit.dart';

class UpdateProfileD extends StatefulWidget {
  const UpdateProfileD({Key? key}) : super(key: key);

  @override
  State<UpdateProfileD> createState() => _UpdateProfileDState();
}

class _UpdateProfileDState extends State<UpdateProfileD> {
  ...
  Future pickImagee()async{
    try {
      ...
      final imageTemporary = File(image.path);
      setState(() {
        this.certificate=imageTemporary;
...
//------------------------------------
                  ...
                  BlocConsumer<UpdateProfileDocCubit, UpdateProfileDocState>(
                    listener: (context, state) {
                      if (state is UpdateProfileDocFailed) {
                      ...
                      } else if (state is UpdateProfileDocSuccess) {
                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                          content: Text(
                            "تم تحديث عضويتك بنجاح ",
                          ),
                          duration: Duration(
                            seconds: 3,
                          ),
                    ...
                    builder: (context, state) {
                      return state is UpdateProfileDocLoading
                          ? Center(
                        ...
                        onPressed: () {
                          if (formkey.currentState!.validate()) {
                            UpdateProfileDocCubit.get(context).update(
                              name: name.text,
                              email: email.text,
